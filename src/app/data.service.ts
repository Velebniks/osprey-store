...
export class DataService {
  ...
  constructor() {
    let storedBackpacks = localStorage.getItem('backpacks');
    if (storedBackpacks) {
      this.backpacks = JSON.parse(storedBackpacks);
    } else {
      this.backpacks = [
        {
          id: 1,
  ...
  toggleInCart(backpack) {
    backpack.inCart = !backpack.inCart;
    localStorage.setItem('backpacks', JSON.stringify(this.backpacks));
  }
}
